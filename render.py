...
class Canvas(app.Canvas):
	...
	def camera_position(self):
		...
		rotation = util.transforms.rotate(angle=numpy.degrees(self.latitude), axis=[1,0,0])
		rotation = util.transforms.rotate(angle=numpy.degrees(self.longitude), axis=[0,1,0])
	...
	def on_mouse_press(self, event):
		if event.button == 2:
			...
			if depth >= 1.0:
				...
				invmat = util.transforms.translate([0.0, 0.0, self.distance])
				invmat = util.transforms.rotate(numpy.degrees(self.latitude), [1, 0, 0])
				
				invmat = util.transforms.rotate(numpy.degrees(self.longitude), [0, 1, 0])
				invmat = util.transforms.translate([self.center[0], self.center[1], self.center[2]])
	...
	def on_mouse_move(self, event):
		if event.is_dragging:
			...
			if event.button == 3:
				rotation = numpy.identity(4)
				rotation = util.transforms.rotate(numpy.degrees(self.latitude), [1, 0, 0])
				rotation = util.transforms.rotate(numpy.degrees(self.longitude), [0, 1, 0])
	...
	def on_draw(self, event):
		...
		view = util.transforms.translate(offset=[-self.center[0], -self.center[1], -self.center[2]])
		view = util.transforms.rotate(axis=[0, 1, 0], angle=-numpy.degrees(self.longitude))
		view = util.transforms.rotate(axis=[1, 0, 0], angle=-numpy.degrees(self.latitude))
		view = util.transforms.translate(offset=[0.0, 0.0, -self.distance])
